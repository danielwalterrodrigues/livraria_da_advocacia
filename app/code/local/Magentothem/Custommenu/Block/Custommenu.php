...
class Magentothem_Custommenu_Block_Custommenu extends Mage_Catalog_Block_Navigation
{
    ...
    public function drawCustomMenuItem($category, $level = 0, $last = false,$item)
    {
        ...
        $nomecat = $this->escapeHtml($category->getName());
        $html[] = '<div class="parentMenu ehTOP">';
        $html[] = '<a href="'.$this->getCategoryUrl($category).'">';
        $name = $this->escapeHtml($category->getName());
        $name = str_replace(' ', '&nbsp;', $name);
        $namelink = str_replace($what,$by, $name);
        $namelink = strtolower($namelink);
        $html[] = "<div class='cat_imagem_top'><img class='cat_imagem' src=" . Mage::getBaseUrl('media') . 'catalog/category/' . $namelink . ".png></div>";
        $html[] = '<div class="cat_imagem_bottom"><span>' . $name . '</span></div>';
        $html[] = '</a>';
        $html[] = '</div>';
        ...
        if ($drawPopup)
        {
            ...
            $html[] = '<div id="popup' . $id . '" class="popup" style="display: none; width: 1228px;  top:90px !important;">';
            // --- draw Sub Categories ---
...
//        $html[] = '</a>';
        ...
        {
            // --- Popup function for hide ---
            $html[] = '<div id="popup' . $id . '" class="popup cmsblock" style="display: none; width: 904px; top:90px !important;">';
            if ($blockHtml)
            {
                $html[] = '<div class="block2" id="block2' . $id . '">';
                $html[] = $blockHtml;
