		...
		if(count($results) > 0) {
			if(count($productIds) > 0) {
				...
				foreach($productIds as $productId) {
					...
					$pricespecial = $_coreHelper->currency($product->getFinalPrice(),true,false);
					if($pricespecial==$price) {
						$precofinal = $price;
						} else {
						$precofinal = "<div style='font-size:10px;'>De: ".$price. "</div> Por: ".$pricespecial."</div>";
					}
					...
					$html .= "<div class='product-price'><h5>".$pricespecial."</h5></div>";
					$html .= "</div>";
					$html .= "</li>";
					$resultCount++;
